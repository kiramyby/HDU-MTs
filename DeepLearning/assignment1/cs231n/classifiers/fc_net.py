...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        hidden, cache_hidden = affine_relu_forward(
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, len(dims)):
            ...
            if self.normalization in {"batchnorm", "layernorm"} and i < len(dims) - 1:
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                scores, norm_cache = batchnorm_forward(
                    scores, gamma, beta, self.bn_params[layer - 1]
                )
            elif self.normalization == "layernorm":
                gamma = self.params[f"gamma{layer}"]
                beta = self.params[f"beta{layer}"]
                scores, norm_cache = layernorm_forward(
                    scores, gamma, beta, self.bn_params[layer - 1]
                )
        ...
        scores, final_cache = affine_forward(scores, W_final, b_final)
        ...
        for layer in reversed(range(1, self.num_layers)):
            affine_cache, norm_cache, relu_cache, dropout_cache = caches[layer - 1]
